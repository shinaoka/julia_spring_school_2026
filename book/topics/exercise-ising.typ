...
モンテカルロ法では、連続するステップの測定値は *相関* している（前のステップのスピン配置を少し変えただけなので）。この相関の強さを特徴づけるのが *自己相関時間* `τ`。

- `τ` が大きい → 連続する測定値が似通っている → 独立なサンプル数が少ない
- 標準誤差を正しく評価するには、この相関を考慮する必要がある

=== binning で自己相関の影響を見る

π 推定では各ビンが独立だったが、イジング模型では連続するステップに相関がある。ビンサイズ `B` を変えて標準誤差の変化を見ると、自己相関の影響が分かる：
