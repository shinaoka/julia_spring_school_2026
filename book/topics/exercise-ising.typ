...
2次元強磁性イジング模型は、格子上のスピン（+1 または −1）が最近接で相互作用する模型。温度を変えると *相転移* が起きる：

- *低温*：スピンが揃う（強磁性相）→ 磁化が大きい
- *高温*：スピンがバラバラ（常磁性相）→ 磁化がほぼゼロ
- *転移温度*（2D 正方格子）：`Tc ≈ 2.269`（厳密解あり）

磁化の二乗平均 `⟨M²⟩` を温度の関数として計算すると、相転移が目に見える。

== マルコフ連鎖モンテカルロ法（MCMC）

前の演習（π 推定）では、乱数点を *独立に* 生成してサンプリングした。しかし、イジング模型のような多体系では、ボルツマン分布 `W(Σ) = Z⁻¹ exp(−βE(Σ))` に従う配置を直接生成することは困難。そこで *マルコフ連鎖モンテカルロ法（MCMC）* を使う。

=== 基本的な考え方

MCMC では、状態の *時系列*（マルコフ連鎖）を構成する：

+ 現在の状態 `Σ` から、遷移確率 `T(Σ → Σ')` に従って次の状態 `Σ'` を選ぶ
+ これを繰り返すと、十分な時間の後に定常分布 `W(Σ)` に収束する

この収束を保証するために、遷移確率は以下の *詳細釣り合い条件* を満たす必要がある：

```
T(Σ → Σ') W(Σ) = T(Σ' → Σ) W(Σ')    （すべての Σ, Σ' に対して）
```

直観的には「`Σ → Σ'` に行く流れ」と「`Σ' → Σ` に戻る流れ」が等しいという条件。
...
+ エネルギー変化 `ΔE` を計算する
+ 受理確率 `min(1, exp(−βΔE))` で提案を受理する：
  - `ΔE ≤ 0`（エネルギーが下がる）→ 必ず受理
  - `ΔE > 0`（エネルギーが上がる）→ 確率 `exp(−βΔE)` で受理

この規則が詳細釣り合いを満たすことは、`T(Σ → Σ') / T(Σ' → Σ) = exp(−β(E(Σ') − E(Σ))) = W(Σ') / W(Σ)` から確認できる。

=== π 推定との決定的な違い：自己相関

π 推定では各サンプルが *独立* だったので、標準誤差は単純に `σ / √N` で計算できた。

MCMC では、連続する状態が *相関* している（前の配置を少し変えただけなので）。この相関の強さを *自己相関時間 `τ`* と呼ぶ。

- `τ` が大きい → 連続する測定値が似通っている → *実効的な独立サンプル数* が `N` より大幅に少ない
- 真の標準誤差 ≈ `σ / √(N / 2τ)` （`τ` を無視すると標準誤差を *過小評価* してしまう）
- 特に *転移温度付近* では `τ` が大きくなる（*臨界減速*）
...
== 課題 2：Accumulator で `⟨M²⟩` を計算する

前の演習で作った `Accumulator` をここで再利用する。各ステップの磁化 `M` に対して `M²` を蓄積し、平均と標準誤差を求めよう。
...
複数の温度（逆温度 `β`）で `⟨M²⟩` を計算し、相転移を観察しよう。
...
- `β < 0.44`（高温）：`⟨M²⟩` は小さい（スピンがバラバラ）
- `β > 0.44`（低温）：`⟨M²⟩` は 1 に近づく（スピンが揃う）
- `β ≈ 0.44` 付近：急激に変化する（相転移）
...
+ ビンサイズ `B` が自己相関時間 `τ` より十分大きければ、ビン平均同士はほぼ *独立* になる
