...
#show: book-page.with(title: "発展演習：2次元イジング模型のモンテカルロ法")

= 発展演習：2次元イジング模型のモンテカルロ法

講義で到達しなくても構いません。π 推定で学んだモンテカルロ法と Accumulator を、物理の問題に応用する発展課題です。
...
$ H = -J sum_(chevron.l i, j chevron.r) S_i S_j $

で定義される。ここで：

- $S_i = plus.minus 1$：格子点 $i$ 上のスピン変数
- $J > 0$：強磁性結合定数（隣り合うスピンが揃うとエネルギーが下がる）
- $sum_(chevron.l i, j chevron.r)$：最近接ペアの和（2D 正方格子では各スピンに 4 つの最近接サイト）
...
- *転移温度*（2D 正方格子）：$T_c = 2 J \/ ln(1+sqrt(2)) approx 2.269$（Onsager の厳密解）

磁化の二乗平均 $chevron.l M^2 chevron.r$ を温度の関数として計算すると、相転移が目に見える。

== マルコフ連鎖モンテカルロ法（MCMC）

前の演習（π 推定）では、乱数点を *独立に* 生成してサンプリングした。しかし、イジング模型のような多体系では、ボルツマン分布 $W(Sigma) = Z^(-1) exp(-beta E(Sigma))$ に従う配置を直接生成することは困難。そこで *マルコフ連鎖モンテカルロ法（MCMC）* を使う。

=== 基本的な考え方

MCMC では、状態の *時系列*（マルコフ連鎖）を構成する：

+ 現在の状態 $Sigma$ から、遷移確率 $T(Sigma arrow Sigma')$ に従って次の状態 $Sigma'$ を選ぶ
+ これを繰り返すと、十分な時間の後に定常分布 $W(Sigma)$ に収束する

この収束を保証するために、遷移確率は以下の *詳細釣り合い条件* を満たす必要がある：

$ T(Sigma arrow Sigma') W(Sigma) = T(Sigma' arrow Sigma) W(Sigma') quad (forall Sigma, Sigma') $

直観的には「$Sigma arrow Sigma'$ に行く流れ」と「$Sigma' arrow Sigma$ に戻る流れ」が等しいという条件。

=== メトロポリス法

詳細釣り合いを満たす代表的なアルゴリズムが *メトロポリス法*。手順は：

+ スピンを 1 つランダムに選び、反転を *提案* する
+ エネルギー変化 $Delta E$ を計算する
+ 受理確率 $min(1, exp(-beta Delta E))$ で提案を受理する：
  - $Delta E lt.eq 0$（エネルギーが下がる）→ 必ず受理
  - $Delta E > 0$（エネルギーが上がる）→ 確率 $exp(-beta Delta E)$ で受理

この規則が詳細釣り合いを満たすことは、$T(Sigma arrow Sigma') / T(Sigma' arrow Sigma) = exp(-beta (E(Sigma') - E(Sigma))) = W(Sigma') / W(Sigma)$ から確認できる。

=== π 推定との決定的な違い：自己相関

π 推定では各サンプルが *独立* だったので、標準誤差は単純に $sigma / sqrt(N)$ で計算できた。

MCMC では、連続する状態が *相関* している（前の配置を少し変えただけなので）。この相関の強さを *自己相関時間 $tau$* と呼ぶ。

- $tau$ が大きい → 連続する測定値が似通っている → *実効的な独立サンプル数* が $N$ より大幅に少ない
- 真の標準誤差 ≈ $sigma / sqrt(N / 2 tau)$（$tau$ を無視すると標準誤差を *過小評価* してしまう）
- 特に *転移温度付近* では $tau$ が大きくなる（*臨界減速*）
...
== 課題 2：Accumulator で $chevron.l M^2 chevron.r$ を計算する
...
複数の温度（逆温度 $beta$）で $chevron.l M^2 chevron.r$ を計算し、相転移を観察しよう。
...
- $beta < 0.44$（高温）：$chevron.l M^2 chevron.r$ は小さい（スピンがバラバラ）
- $beta > 0.44$（低温）：$chevron.l M^2 chevron.r$ は 1 に近づく（スピンが揃う）
...
- `L` を倍にすると計算時間はおよそ何倍になる？
- 1 回のスピン更新（近接 4 サイトの参照 + Metropolis 判定）の計算量は `L` に依存するか？
- 以上から、1 MC step の計算時間は `Nspin` の何乗に比例するか？

=== 答え

1 回のスピン更新は O(1)（近接サイトの数は常に 4）。1 MC step で `L²` 回試行するので、計算時間は *`Nspin` の 1 乗に比例*（線形）。`@btime` の結果もこれを裏付けるはず。
...
binning 解析は、この問題を解決するシンプルな方法：

+ 連続する $B$ ステップの測定値をまとめて 1 つの *ビン平均* を作る
+ ビンサイズ $B$ が自己相関時間 $tau$ より十分大きければ、ビン平均同士はほぼ *独立* になる
+ 独立なビン平均に対して `Accumulator` を使えば、正しい標準誤差が得られる
...
- 高温（`β = 0.2`）と転移点付近（`β = 0.44`）で、飽和するビンサイズはどう違う？
...
- Accumulator で $chevron.l M^2 chevron.r$ を計算できた
- 温度を変えて相転移が観察できた
