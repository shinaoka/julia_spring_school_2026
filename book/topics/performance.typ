...
`BenchmarkTools.jl` の `@btime` は、内部で何度も繰り返し実行し、コンパイルを除いた安定した実行時間を報告してくれる。
...
講師が用意したサンプルリポジトリには、わざと遅く書いた `estimate_pi()` が入っています。これを clone して動かしましょう。
...
出力される実行時間を確認し、「これは速い？遅い？」を考えてみましょう。

== LLM と一緒に高速化する

ここからが本題です。以下のサイクルを回します。
