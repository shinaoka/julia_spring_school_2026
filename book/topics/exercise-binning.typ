...
#show: book-page.with(title: "演習：binning で π の推定精度を評価する")

= 演習：binning で π の推定精度を評価する

これまでに学んだ構造体・関数・多重ディスパッチを組み合わせて、Monte Carlo 推定の精度を定量的に評価する演習です。

== 背景：1 回の推定だけでは不十分

`estimate_pi(N)` を 1 回実行すると値は出るが、「どのくらい信頼できるか」が分からない。そこで *binning*（ビニング）を使う：

+ 全体の試行数 `N` を `M` 個の *ビン* に分割する（各ビンのサイズ `B = N ÷ M`）
+ 各ビンで独立に π を推定する
+ ビンごとの推定値を蓄積して、*平均* と *標準誤差* を計算する
