...
- Google Gemini Code Assist の設定（Copilot の代替。個人利用は無料）
  1. **VS Code 拡張をインストール**：拡張機能で `Gemini Code Assist` を検索してインストール
  2. **Google アカウントでログイン**：サイドバーの Gemini パネルからサインインする
  3. **repository context を使う**：チャット入力欄で `@` を使い、対象ファイルやフォルダを文脈に追加する
  4. **IntelliJ 系 IDE を使う場合**：Plugins Marketplace で `Gemini Code Assist` を入れてサインインする
  - 公式ドキュメント：
